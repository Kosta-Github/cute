...
#define CUTE_DETAIL_ASSERT(EXPR_EVAL, ...)                                                                                          \
    try {                                                                                                                           \
        ++cute::test_suite_result::current().checks_performed;                                                                      \
        if(!(EXPR_EVAL)) {                                                                                                          \
            throw cute::detail::exception("test failed", __VA_ARGS__);                                                              \
        }                                                                                                                           \
    } catch(cute::detail::exception const&) {                                                                                       \
        throw;                                                                                                                      \
    } catch(std::exception const &ex) {                                                                                             \
        throw cute::detail::exception("got an unexpected exception with message \"" + std::string(ex.what()) + "\"", __VA_ARGS__);  \
    } catch(...) {                                                                                                                  \
        throw cute::detail::exception("got an unexpected exception of unknown type", __VA_ARGS__);                                  \
    }
...
#define CUTE_ASSERT(EXPR, ...)              CUTE_DETAIL_ASSERT(EXPR,                 __FILE__, __LINE__, CUTE_CAPTURE(EXPR),     __VA_ARGS__)
#define CUTE_ASSERT_THROWS_NOT(EXPR)        CUTE_DETAIL_ASSERT(((void)(EXPR), true), __FILE__, __LINE__, cute::capture(#EXPR, ""))
