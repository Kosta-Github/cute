...
#define CUTE_DETAIL_EXPECT_THROWS_AS(EXPR, EXCEPT) \
    for(;;) { \
        try { \
            ++cute::test_suite_result::current().checks_performed; \
            (void)(EXPR); \
...
#define CUTE_EXPECT_THROWS(EXPR) CUTE_DETAIL_EXPECT_THROWS_AS(EXPR, std::exception)
#define CUTE_EXPECT_THROWS_AS(EXPR, EXCEPT) CUTE_DETAIL_EXPECT_THROWS_AS(EXPR, EXCEPT)
