...
#define CUTE_DETAIL_ASSERT(EXPR_EVAL, FILE, LINE, CAPS1, CAP2)                                                                                  \
    try {                                                                                                                                       \
        ++cute::test_suite_result::current().checks_performed;                                                                                  \
        if(!(EXPR_EVAL)) {                                                                                                                      \
            throw cute::detail::exception("test failed", FILE, LINE, CAPS1, CAP2);                                                              \
        }                                                                                                                                       \
    } catch(cute::detail::exception const&) {                                                                                                   \
        throw;                                                                                                                                  \
    } catch(std::exception const &ex) {                                                                                                         \
        throw cute::detail::exception("got an unexpected exception with message \"" + std::string(ex.what()) + "\"", FILE, LINE, CAPS1, CAP2);  \
    } catch(...) {                                                                                                                              \
        throw cute::detail::exception("got an unexpected exception of unknown type", FILE, LINE, CAPS1, CAP2);                                  \
    }
...
#define CUTE_DETAIL_ASSERT_THROWS_AS(EXPR, EXCEPT)                                                                                                  \
    for(;;) {                                                                                                                                       \
        ...
        throw cute::detail::exception("didn't get an expected exception of type \"" #EXCEPT "\"", __FILE__, __LINE__, cute::capture(#EXPR, ""));    \
    }
...
#define CUTE_ASSERT(EXPR, ...)              CUTE_DETAIL_ASSERT(EXPR,                 __FILE__, __LINE__, CUTE_CAPTURE(EXPR), cute::captures(__VA_ARGS__))
#define CUTE_ASSERT_THROWS_NOT(EXPR)        CUTE_DETAIL_ASSERT(((void)(EXPR), true), __FILE__, __LINE__, cute::capture(#EXPR, ""), cute::captures())
